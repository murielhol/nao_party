...
class LandmarkData:
    def __init__(self):
        ...
        self.image_dir_cali = "./calibration_data/"
        ...
        self.file_seq = [i for i in range(198)]
    ...
    def _get_landmark_dataset(self, file_seq, outfile="./landmark_data.json"):
        ...
        for i in file_seq:
            ...
            if len(landmark_info) > 0:
                ...
                robot_pos = { 'head_yaw': head_yaw }
                landmarks.append(robot_pos)
                lm_width = [landmark_info[2]]
                if len(landmark_info) > 4:
                    lm_width.append(landmark_info[6])

                for k, lm in enumerate(lm_width):     # assume the width is stored as a list

                    id = landmark_info[1+k*4][0]
                    distance = self._find_distance(lm)
                    bearing = self._find_bearing(landmark_info[3+k*4][0], i)  # use index i to find corresponding log
